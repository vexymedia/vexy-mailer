...
`npm test` runs 255 tests. Most are ordinary unit tests, but the interesting
...
55 checks driving the real UI: sign-in, mailbox setup and connection test, CSV
import, sequence editing, campaign start, a worker tick, suppression, the
unsubscribe link under every HTTP method, and the cron endpoint's
