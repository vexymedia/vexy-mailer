...
const invalid = () => page("Link not valid", `<p>This unsubscribe link is not valid.</p>`, 400);
...
export async function GET(_request: Request, { params }: Context): Promise<Response> {
  ...
  return page(
    "Unsubscribe",
    `<p>Confirm that <strong>${escapeHtml(email)}</strong> should no longer receive emails from us.</p>` +
...
export async function POST(_request: Request, { params }: Context): Promise<Response> {
  ...
  return page(
    "Unsubscribed",
    `<p>${escapeHtml(email)} has been removed. You will not receive any further emails from us.</p>`,
    200,
  );
}
